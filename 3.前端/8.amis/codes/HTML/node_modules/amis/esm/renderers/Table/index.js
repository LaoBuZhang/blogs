...
import { resolveVariableAndFilter, getScrollParent, offset, getStyleNumber, isObject, anyChanged, isPureVariable, isArrayChildrenModified, createObject, difference, resizeSensor, eachTree, resolveVariable, filter, padArr, noop, evalExpression, ScopedContext, autobind, Renderer, TableStore } from 'amis-core';
...
var Table = /** @class */ (function (_super) {
    ...
    Table.prototype.componentDidMount = function () {
        ...
        var parent = getScrollParent(currentNode);
    ...
    Table.prototype.updateAutoFillHeight = function () {
        ...
        var height = isObject(autoFillHeight)
    ...
    Table.prototype.componentDidUpdate = function (prevProps) {
        ...
        Promise.resolve().then(function () { return _this.updateTableInfoLazy(); });
    };
    ...
    Table.prototype.updateTableInfo = function () {
        ...
        forEach(table.querySelectorAll('tbody>tr>*:last-child'), 
        /**
         * ! 弹窗中的特殊说明
    ...
    Table.prototype.renderAutoFilterForm = function () {
        ...
        var body = [];
        padArr(activedSearchableColumns, 3, true).forEach(function (group) {
            var children = [];
            group.forEach(function (column) {
                var _a, _b, _c, _d;
                children.push(column
                    ? __assign(__assign({}, (column.searchable === true
                        ? {
                            type: 'input-text',
                            name: column.name,
                            label: column.label
        ...
        return render('searchable-form', {
            ...
            actions: [
                ...
                showExpander
                    ? {
                        children: function () { return (React.createElement("a", { className: cx('Table-SFToggler', store.searchFormExpanded ? 'is-expanded' : ''), onClick: store.toggleSearchFormExpanded },
    ...
    Table.prototype.renderHeadCell = function (column, props) {
        ...
        if (column.type === '__checkme') {
            return (React.createElement("th", __assign({}, props, { className: cx(column.pristine.className) }), store.rows.length && multiple ? (React.createElement(Checkbox, { classPrefix: ns, partial: store.someChecked && !store.allChecked, checked: store.someChecked, disabled: store.isSelectionThresholdReached, onChange: this.handleCheckAll })) : ('\u00A0')));
        }
        ...
        if (column.searchable && column.name && !autoGenerateFilter) {
            affix.push(React.createElement(HeadCellSearchDropDown, __assign({ key: "table-head-search" }, this.props, { onQuery: onQuery, name: column.name, searchable: column.searchable, sortable: false, type: column.type, data: query, orderBy: store.orderBy, orderDir: store.orderDir, popOverContainer: this.getPopOverContainer })));
        }
        if (column.sortable && column.name) {
            affix.push(React.createElement("span", { key: "table-head-sort", className: cx('TableCell-sortBtn'), onClick: function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        if (!column.searchable && column.filterable && column.name) {
            affix.push(React.createElement(HeadCellFilterDropDown, __assign({ key: "table-head-filter" }, this.props, { onQuery: onQuery, name: column.name, type: column.type, data: query, filterable: column.filterable, popOverContainer: this.getPopOverContainer })));
        }
    ...
    Table.prototype.renderAffixHeader = function (tableClassName) {
        ...
        return affixHeader ? (React.createElement(React.Fragment, null,
            ...
            React.createElement("div", { className: cx('Table-fixedTop-shadow') }))) : null;
    };
    Table.prototype.renderFixedColumns = function (rows, columns, headerOnly, tableClassName) {
        ...
        var hideHeader = store.filteredColumns.every(function (column) { return !column.label; });
        var columnsGroup = store.columnGroup;
        return (React.createElement("table", { className: cx('Table-table', tableClassName, {
            ...
            headerOnly ? null : !rows.length ? (React.createElement("tbody", null,
                React.createElement("tr", { className: cx('Table-placeholder') },
                    React.createElement("td", { colSpan: columns.length }, render('placeholder', translate(placeholder || 'placeholder.noData')))))) : (React.createElement(TableBody, { tableClassName: cx(tableClassName, {
                    'Table-table--withCombine': store.combineNum > 0
    ...
    Table.prototype.render = function () {
        var _a = this.props, className = _a.className, style = _a.style, store = _a.store, cx = _a.classnames, affixColumns = _a.affixColumns, autoFillHeight = _a.autoFillHeight, autoGenerateFilter = _a.autoGenerateFilter;
