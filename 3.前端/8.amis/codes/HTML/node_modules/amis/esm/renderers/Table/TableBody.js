...
var TableBody = /** @class */ (function (_super) {
    ...
    TableBody.prototype.renderRows = function (rows, columns, rowProps) {
        ...
        return rows.map(function (item, rowIndex) {
            ...
            var doms = [
                React.createElement(TableRow, __assign({}, itemProps, { itemAction: itemAction, classnames: cx, checkOnItemClick: checkOnItemClick, key: item.id, itemIndex: rowIndex, item: item, itemClassName: cx(rowClassNameExpr
            ...
            if (footable && footableColumns.length) {
                if (item.depth === 1) {
                    doms.push(React.createElement(TableRow, __assign({}, itemProps, { itemAction: itemAction, classnames: cx, checkOnItemClick: checkOnItemClick, key: "foot-".concat(item.id), itemIndex: rowIndex, item: item, itemClassName: cx(rowClassNameExpr
                            ? filter(rowClassNameExpr, item.data)
                ...
                // 嵌套表格
                doms.push.apply(doms, _this.renderRows(item.children, columns, __assign(__assign({}, rowProps), { parent: item })));
            }
            return doms;
        });
    };
    TableBody.prototype.renderSummaryRow = function (position, items, rowIndex) {
        ...
        for (var index = 0; index < filterColumns.length; index++) {
            ...
            item && result.push(__assign({}, item));
        }
        //  如果是勾选栏，让它和下一列合并。
        ...
        if (appendLen) {
            var item = result.pop();
            result.push(__assign(__assign({}, item), { colSpan: (item.colSpan || 1) + appendLen }));
        }
        ...
        return (React.createElement("tr", { className: cx('Table-tr', 'is-summary', position === 'prefix' ? prefixRowClassName : '', position === 'affix' ? affixRowClassName : ''), key: "summary-".concat(position, "-").concat(rowIndex || 0) }, result.map(function (item, index) {
            ...
            return (React.createElement(Com, { key: index, colSpan: item.colSpan, className: item.cellClassName }, render("summary-row/".concat(index), item, {
                data: ctx
            })));
        })));
    };
