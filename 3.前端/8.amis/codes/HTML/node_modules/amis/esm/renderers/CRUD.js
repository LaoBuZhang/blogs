...
import { parseQuery, getPropValue, anyChanged, isArrayChildrenModified, isObjectShallowModified, isApiOutdated, isPureVariable, resolveVariableAndFilter, filter, createObject, isVisible, str2function, qsparse, qsstringify, isEffectiveApi, evalExpression, extendObject, getExprProperties, getVariable, ScopedContext, Renderer, CRUDStore } from 'amis-core';
import { Select, Button, Icon, Html } from 'amis-ui';
...
var CRUD = /** @class */ (function (_super) {
    ...
    CRUD.prototype.handleBulkAction = function (selectedItems, unSelectedItems, e, action) {
        ...
        var ids = selectedItems
            ...
            .join(',');
        var ctx = createObject(store.mergedData, __assign(__assign({}, selectedItems[0]), { currentPageData: store.mergedData.items.concat(), rows: selectedItems, items: selectedItems, selectedItems: selectedItems, unSelectedItems: unSelectedItems, ids: ids }));
        var fn = function () {
            if (action.actionType === 'dialog') {
        ...
        if (!action.ignoreConfirm &&
            ...
            env.confirm(confirmText).then(function (confirmed) { return confirmed && fn(); });
        }
        else {
            fn();
        }
    ...
    CRUD.prototype.handleFilterSubmit = function (values, jumpToFirstPage, replaceLocation, search) {
        ...
        var _d = this.props, store = _d.store, syncLocation = _d.syncLocation, env = _d.env, pageField = _d.pageField, perPageField = _d.perPageField, loadDataOnceFetchOnFilter = _d.loadDataOnceFetchOnFilter;
        ...
        isEffectiveApi(api, data)
            ? store
                ...
                if (!store.data.items.length &&
                    ...
                    page > 1 &&
                    lastPage < page) {
                    _this.search(__assign(__assign({}, store.query), (_a = {}, _a[pageField || 'page'] = lastPage, _a)), false, undefined);
                }
                interval &&
                    _this.mounted &&
                    (!stopAutoRefreshWhen ||
    ...
    CRUD.prototype.renderPagination = function (toolbar) {
        ...
        if (typeof toolbar !== 'string') {
            ...
            extraProps.popOverContainerSelector = toolbar.popOverContainerSelector;
        }
        else {
        ...
        if (type === 'bulkActions' || type === 'bulk-actions') {
            ...
            if (len) {
                return (React.createElement("div", { className: cx_1('Crud-toolbar'), key: index }, children.map(function (_a, index) {
                    ...
                    return (React.createElement("div", { key: index, className: cx_1('Crud-toolbar-item', align ? "Crud-toolbar-item--".concat(align) : ''
                        // toolbar.className
                        ) }, child));
                })));
            }
