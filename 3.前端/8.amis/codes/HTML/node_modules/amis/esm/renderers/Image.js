...
import { autobind, themeable, localeable, handleAction, filter, getPropValue, Renderer } from 'amis-core';
...
var ImageThumb = /** @class */ (function (_super) {
    ...
    ImageThumb.prototype.render = function () {
        ...
        var enlarge = enlargeAble || overlays ? (React.createElement("div", { key: "overlay", className: cx('Image-overlay') },
            enlargeAble ? (React.createElement("a", { "data-tooltip": __('Image.zoomIn'), "data-position": "bottom", target: "_blank", onClick: this.handleEnlarge },
                React.createElement(Icon, { icon: "view", className: "icon" }))) : null,
            overlays)) : null;
        var image = (React.createElement("div", { className: cx('Image', imageMode === 'original' ? 'Image--original' : 'Image--thumb', className), style: href ? undefined : style },
            ...
            title || caption ? (React.createElement("div", { key: "caption", className: cx('Image-info') },
                title ? (React.createElement("div", { className: cx('Image-title'), title: title }, title)) : null,
                caption ? (React.createElement("div", { className: cx('Image-caption'), title: caption }, caption)) : null)) : null));
        if (href) {
...
var ImageField = /** @class */ (function (_super) {
    ...
    ImageField.prototype.render = function () {
        ...
        var value = finnalSrc || getPropValue(this.props) || defaultImage || imagePlaceholder;
        var finnalHref = href ? filter(href, data, '| raw') : '';
        return (React.createElement("div", { className: cx('ImageField', imageMode === 'original'
                ? 'ImageField--original'
                : 'ImageField--thumb', className), style: style, onClick: this.handleClick }, value ? (React.createElement(ThemedImageThumb, { className: innerClassName, imageClassName: imageClassName, thumbClassName: thumbClassName, height: height, width: width, src: value, href: finnalHref, title: filter(title, data), caption: filter(imageCaption, data), thumbMode: thumbMode, thumbRatio: thumbRatio, originalSrc: (_a = filter(originalSrc, data, '| raw')) !== null && _a !== void 0 ? _a : value, enlargeAble: enlargeAble && value !== defaultImage, onEnlarge: this.handleEnlarge, imageMode: imageMode })) : (React.createElement("span", { className: "text-muted" }, placeholder))));
    };
