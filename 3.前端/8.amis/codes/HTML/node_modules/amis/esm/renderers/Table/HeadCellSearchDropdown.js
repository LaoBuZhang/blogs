...
import { setVariable, createObject, Overlay, PopOver } from 'amis-core';
...
var HeadCellSearchDropDown = /** @class */ (function (_super) {
    ...
    HeadCellSearchDropDown.prototype.render = function () {
        ...
        return (React.createElement("span", { className: cx("".concat(ns, "TableCell-searchBtn"), isActive ? 'is-active' : '', this.state.isOpened ? 'is-opened' : '') },
            React.createElement("span", { onClick: this.open },
                React.createElement(Icon, { icon: "search", className: "icon" })),
            this.state.isOpened ? (React.createElement(Overlay, { container: popOverContainer || (function () { return findDOMNode(_this); }), placement: "left-bottom-left-top right-bottom-right-top", target: popOverContainer ? function () { return findDOMNode(_this).parentNode; } : null, show: true },
                React.createElement(PopOver, { classPrefix: ns, onHide: this.close, className: cx("".concat(ns, "TableCell-searchPopOver"), searchable.className), overlay: true }, render('quick-search-form', formSchema, {
                    data: __assign(__assign({}, data), { orderBy: orderBy, orderDir: orderBy === name ? store.orderDir : '' }),
