...
import { filter, isPureVariable, resolveVariableAndFilter, getPropValue, autobind, Renderer } from 'amis-core';
...
var ImagesField = /** @class */ (function (_super) {
    ...
    ImagesField.prototype.render = function () {
        ...
        this.list = list;
        return (React.createElement("div", { className: cx('ImagesField', className), style: style }, Array.isArray(list) ? (React.createElement("div", { className: cx('Images', listClassName) }, list.map(function (item, index) { return (React.createElement(ThemedImageThumb, { index: index, className: cx('Images-item'), key: index, src: (src ? filter(src, item, '| raw') : item && item.image) ||
                item, originalSrc: (originalSrc
                ? filter(originalSrc, item, '| raw')
                : item && item.src) || item, title: item && item.title, caption: item && (item.description || item.caption), thumbMode: thumbMode, thumbRatio: thumbRatio, enlargeAble: enlargeAble, onEnlarge: _this.handleEnlarge, showToolbar: showToolbar, toolbarActions: toolbarActions })); }))) : defaultImage ? (React.createElement("div", { className: cx('Images', listClassName) },
