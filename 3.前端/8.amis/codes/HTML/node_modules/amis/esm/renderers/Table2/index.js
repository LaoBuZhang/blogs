...
import { __extends, __spreadArray, __rest, __assign, __awaiter, __generator, __decorate, __metadata } from 'tslib';
import React from 'react';
import { findDOMNode } from 'react-dom';
import { isAlive } from 'mobx-state-tree';
import cloneDeep from 'lodash/cloneDeep';
import isEqual from 'lodash/isEqual';
import { resolveVariableAndFilter, evalExpression, anyChanged, isPureVariable, isObject, resolveVariable, filter, isEffectiveApi, createObject, difference, ScopedContext, autobind, Renderer, TableStore2 } from 'amis-core';
import { Table, Icon, Spinner } from 'amis-ui';
...
var Table2 = /** @class */ (function (_super) {
    ...
    Table2.syncRows = function (store, props, prevProps) {
        ...
        if (props.rowSelection && props.rowSelection.selectedRowKeysExpr) {
            ...
            selectedRowKeys = __spreadArray([], props.rowSelection.selectedRowKeys, true);
        }
        ...
        if (props.expandable && props.expandable.expandedRowKeysExpr) {
            ...
            expandedRowKeys = __spreadArray([], props.expandable.expandedRowKeys, true);
        }
    ...
    Table2.prototype.renderCellSchema = function (schema, props) {
        ...
        if (schema && isObject(schema)) {
            ...
            schema.width; schema.children; schema.title; var rest = __rest(schema, ["width", "children", "title"]);
            return render('cell-field', __assign(__assign({}, rest), { type: 'cell-field', column: rest, data: props.data, name: schema.name }), props);
        }
        return schema;
    };
    Table2.prototype.renderSchema = function (key, schema, props) {
        ...
        if (schema && isObject(schema)) {
            return render(key || 'field', __assign(__assign({}, schema), { data: props.data }), props);
        }
        else if (Array.isArray(schema)) {
            var renderers_1 = [];
            schema.forEach(function (s, i) {
                return renderers_1.push(render(key || 'field', __assign(__assign({}, s), { data: props.data }), __assign(__assign({}, props), { key: i })));
            });
            return renderers_1;
        }
        return schema;
        ...
        Array.isArray(columns) &&
            columns.forEach(function (column, col) {
                ...
                if (column.searchable) {
                    clone.filterDropdown = (React.createElement(HeadCellSearchDropDown, __assign({}, _this.props, { popOverContainer: _this.getPopOverContainer, name: column.name, searchable: column.searchable, orderBy: store.orderBy, orderDir: store.order, data: store.query, key: 'th-search-' + col, store: store })));
                }
                if (column.children) {
    ...
    Table2.prototype.handleQuickChange = function (item, values, saveImmediately, savePristine, options) {
        ...
        onSave
            ...
            : this.handleSave(quickSaveItemApi ? item.data : [item.data], difference(item.data, item.pristine, ['id', primaryField]), [item.path], undefined, item.pristine, options);
    };
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, dispatchEvent('rowClick', createObject(data, { rowItem: rowItem }))];
                    case 1:
                        rendererEvent = _b.sent();
                        if (rendererEvent === null || rendererEvent === void 0 ? void 0 : rendererEvent.prevented) {
                            return [2 /*return*/, rendererEvent === null || rendererEvent === void 0 ? void 0 : rendererEvent.prevented];
    ...
    Table2.prototype.renderTable = function () {
        ...
        if (expandable) {
            ...
            if (expandable && expandable.type) {
                expandableConfig.expandedRowRender = function (record, rowIndex) {
                    return _this.renderSchema('expandableBody', __assign({}, expandable), { data: record });
                };
            }
            if (expandable.expandedRowClassNameExpr) {
                expandableConfig.expandedRowClassName = function (record, rowIndex) { return filter(expandable.expandedRowClassNameExpr, { record: record, rowIndex: rowIndex }); };
                delete expandableConfig.expandedRowClassNameExpr;
            }
        }
        var rowSelectionConfig = null;
        if (rowSelection) {
            rowSelection.selectedRowKeys; var selections = rowSelection.selections, rest_2 = __rest(rowSelection, ["selectedRowKeys", "selections"]);
            rowSelectionConfig = __assign({ selectedRowKeys: store.currentSelectedRowKeys, maxSelectedLength: store.maxKeepItemSelectionLength }, rest_2);
            var disableOn_1 = rowSelection.disableOn;
            rowSelectionConfig.getCheckboxProps = function (record, rowIndex) {
                return {
                    disabled: (disableOn_1
                        ? evalExpression(disableOn_1, { record: record, rowIndex: rowIndex })
                        : false) ||
        ...
        if (rowClassNameExpr) {
            rowClassName = function (record, rowIndex) {
                return filter(rowClassNameExpr, { record: record, rowIndex: rowIndex });
            };
        }
        ...
        return (React.createElement(Table, __assign({}, rest, { onRef: this.getRef, title: this.renderSchema('title', title, { data: this.props.data }), footer: this.renderSchema('footer', footer, { data: this.props.data }), columns: this.buildColumns(store.filteredColumns), dataSource: store.dataSource, rowSelection: rowSelectionConfig, rowClassName: rowClassName, expandable: expandableConfig, footSummary: this.buildSummary('footSummary', footSummary), headSummary: this.buildSummary('headSummary', headSummary), loading: this.renderSchema('loading', loading), placeholder: this.renderSchema('placeholder', placeholder), onSelect: this.handleSelected, onSelectAll: this.handleSelected, onSort: this.handleSort, onFilter: this.handleFilter, onDrag: this.handleOrderChange, itemActions: itemActionsConfig, onRow: __assign(__assign({}, onRow), { onRowClick: this.handleRowClick }) })));
    };
    Table2.prototype.renderHeading = function () {
        ...
        if (store.moved) {
            return (React.createElement("div", { className: cx('Table-heading', headingClassName), key: "heading" }, store.moved ? (React.createElement("span", null,
                __('Table.moved', {
                    moved: store.moved
                }),
    ...
    Table2.prototype.render = function () {
        var _a = this.props, cx = _a.classnames, style = _a.style, _b = _a.loading, loading = _b === void 0 ? false : _b, loadingConfig = _a.loadingConfig;
        this.renderedToolbars = []; // 用来记录哪些 toolbar 已经渲染了
        var heading = this.renderHeading();
        return (React.createElement("div", { className: cx('Table-render-wrapper'), style: style },
