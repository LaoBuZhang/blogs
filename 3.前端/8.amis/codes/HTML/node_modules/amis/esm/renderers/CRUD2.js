...
import { parseQuery, getPropValue, isArrayChildrenModified, isObjectShallowModified, isApiOutdated, isPureVariable, resolveVariableAndFilter, qsparse, qsstringify, createObject, isEffectiveApi, evalExpression, extendObject, filter, getVariable, autobind, ScopedContext, Renderer, CRUDStore } from 'amis-core';
...
var CRUD2 = /** @class */ (function (_super) {
    ...
    function CRUD2(props) {
        ...
        if (syncLocation && location && (location.query || location.search)) {
            store.updateQuery(parseQuery(location), undefined, pageField, perPageField);
        }
        else if (syncLocation && !location && window.location.search) {
            store.updateQuery(parseQuery(window.location), undefined, pageField, perPageField);
        }
    ...
    CRUD2.prototype.componentDidUpdate = function (prevProps) {
        ...
        if (prevProps.syncLocation &&
            ...
            store.updateQuery(parseQuery(props.location), undefined, props.pageField, props.perPageField);
    ...
    CRUD2.prototype.handleSearch = function (data) {
        ...
        var _b = data || {}, query = _b.query, resetQuery = _b.resetQuery, replaceQuery = _b.replaceQuery;
        query =
            syncLocation && query
    ...
    CRUD2.prototype.renderChild = function (region, schema, props) {
        ...
        return render(region, schema, __assign(__assign(__assign({}, props), { 
            // 包两层，主要是为了处理以下 case
            // 里面放了个 form，form 提交过来的时候不希望把 items 这些发送过来。
            // 因为会把数据呈现在地址栏上。
            data: createObject(createObject(store.filterData, store.getData(this.props.data)), {}), render: this.renderChild }), childProps));
    };
    ...
    CRUD2.prototype.render = function () {
        ...
        return (React.createElement("div", { className: cx('Crud2', className, {
            ...
            React.createElement("div", { className: cx('Crud2-toolbar') }, this.renderToolbar('footerToolbar', footerToolbar))));
    };
    CRUD2.propsList = [
        'mode',
        'syncLocation',
