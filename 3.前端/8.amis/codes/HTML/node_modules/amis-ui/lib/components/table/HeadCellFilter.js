...
var HeadCellFilter = /** @class */ (function (_super) {
    ...
    HeadCellFilter.prototype.render = function () {
        ...
        var filterProps = {
            filterDropdown: function (payload) {
                ...
                return options && options.length > 0 ? (React__default["default"].createElement("ul", { className: cx('DropDown-menu') },
                    !column.filterMultiple
                        ? options.map(function (option, index) { return (React__default["default"].createElement("li", { key: index, className: cx({
                                'is-active': option.selected
                            }), onClick: function () {
                                return _this.handleClick(confirm, setSelectedKeys, [option.value]);
                            } }, option.text)); })
                        : options.map(function (option, index) { return (React__default["default"].createElement("li", { key: index },
                            React__default["default"].createElement(Checkbox["default"], { classPrefix: ns, onChange: function (e) {
                                    return _this.handleCheck(confirm, setSelectedKeys, e ? [option.value] : option.value);
                                }, checked: option.selected }, option.text))); }),
                    column.filterMultiple ? (React__default["default"].createElement("li", { key: "dropDown-multiple-menu", className: cx('DropDown-multiple-menu') },
                        React__default["default"].createElement(Button["default"], { size: 'xs', level: 'primary', onClick: function () { return _this.handleConfirmClick(confirm); } }, "\u786E\u5B9A"),
                        React__default["default"].createElement(Button["default"], { size: 'xs', onClick: function () {
        ...
        return (React__default["default"].createElement(HeadCellDropDown["default"], tslib.__assign({ className: "".concat(ns, "TableCell-filterBtn"), layerClassName: "".concat(ns, "TableCell-filterPopOver"), filterIcon: React__default["default"].createElement(icons.Icon, { icon: "column-filter", className: "icon" }), active: column.filtered ||
                (options && options.some(function (item) { return item.selected; })), popOverContainer: popOverContainer ? popOverContainer : function () { return ReactDOM.findDOMNode(_this); }, selectedKeys: this.state.filteredValue }, filterProps)));
    };
    ...
    HeadCellFilter.prototype.handleCheck = function (confirm, setSelectedKeys, selectedKeys) {
        ...
        if (Array.isArray(selectedKeys)) {
            setSelectedKeys && setSelectedKeys(tslib.__spreadArray(tslib.__spreadArray([], filteredValue, true), selectedKeys, true));
        }
        else {
            // 取消选中
            setSelectedKeys &&
