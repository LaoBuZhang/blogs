...
var DatePicker = /** @class */ (function (_super) {
    ...
    DatePicker.prototype.handleChange = function (value) {
        ...
        onChange(utc ? moment__default["default"].utc(value).format(format) : value.format(format));
        if (closeOnSelect && dateFormat && !timeFormat) {
            this.close();
        }
        this.setState({
            inputValue: utc
                ? moment__default["default"].utc(value).format(inputFormat)
    ...
    DatePicker.prototype.inputChange = function (e) {
        ...
        if (value === '') {
            ...
            if (inputCheckRegex.test(value)) {
                var newDate = moment__default["default"](value, inputFormat);
                var dateValue = utc
                    ? moment__default["default"].utc(newDate).format(format)
    ...
    DatePicker.prototype.getAvailableShortcuts = function (key) {
        ...
        for (var i = 0, len = advancedShortcuts.length; i < len; i++) {
            ...
            if (m) {
                return item.resolve.apply(item, tslib.__spreadArray([__], m, true));
            }
        }
        return null;
    };
    DatePicker.prototype.renderShortCuts = function (shortcuts) {
        ...
        var __ = this.props.translate;
        return (React__default["default"].createElement("ul", { className: cx("DatePicker-shortcuts") }, shortcutArr.map(function (item) {
            if (!item) {
                return null;
            }
            var shortcut = {};
            if (typeof item === 'string') {
                ...
                shortcut = tslib.__assign(tslib.__assign({}, item), { date: function () { return item.date; } });
            }
            return (React__default["default"].createElement("li", { className: cx("DatePicker-shortcut"), onClick: function () { return _this.selectRannge(shortcut); }, key: shortcut.key || shortcut.label },
                React__default["default"].createElement("a", null, __(shortcut.label))));
        })));
    };
    DatePicker.prototype.render = function () {
        ...
        var calendarMobile = (React__default["default"].createElement(CalendarMobile["default"], { isDatePicker: true, timeFormat: timeFormat, inputFormat: inputFormat, startDate: date, defaultDate: date, minDate: minDate, maxDate: maxDate, dateFormat: dateFormat, embed: embed, viewMode: viewMode, close: this.close, confirm: this.handleChange, footerExtra: this.renderShortCuts(shortcuts), showViewMode: viewMode === 'quarters' || viewMode === 'months' ? 'years' : 'months', timeConstraints: timeConstraints }));
        ...
        if (embed) {
            ...
            return (React__default["default"].createElement("div", { className: cx("DateCalendar", {
        ...
        return (React__default["default"].createElement("div", { tabIndex: 0, onKeyPress: this.handleKeyPress, onFocus: this.handleFocus, onBlur: this.handleBlur, className: cx("DatePicker", (_a = {
                    'is-disabled': disabled,
            ...
            React__default["default"].createElement(Input["default"], { className: cx('DatePicker-input'), onChange: this.inputChange, onBlur: this.onInputBlur, ref: this.inputRef, placeholder: __(placeholder), autoComplete: "off", value: this.state.inputValue || '', disabled: disabled }),
            clearable && !disabled && normalizeValue(value, format) ? (React__default["default"].createElement("a", { className: cx("DatePicker-clear"), onClick: this.clearValue },
                React__default["default"].createElement(icons.Icon, { icon: "input-clear", className: "icon" }))) : null,
            React__default["default"].createElement("a", { className: cx("DatePicker-toggler") },
                React__default["default"].createElement(icons.Icon, { icon: viewMode === 'time' ? 'clock' : 'date', className: "icon" })),
            !(useMobileUI && amisCore.isMobile()) && isOpened ? (React__default["default"].createElement(amisCore.Overlay, { target: this.getTarget, container: popOverContainer || this.getParent, rootClose: false, placement: overlayPlacement, show: true },
                React__default["default"].createElement(amisCore.PopOver, { classPrefix: ns, className: cx("DatePicker-popover", popoverClassName), onHide: this.close, overlay: true, onClick: this.handlePopOverClick },
            ...
            useMobileUI && amisCore.isMobile() ? (mobileCalendarMode === 'calendar' && useCalendarMobile ? (React__default["default"].createElement(PopUp["default"], { isShow: isOpened, className: cx("".concat(ns, "CalendarMobile-pop")), onHide: this.close, header: CalendarMobileTitle }, calendarMobile)) : (React__default["default"].createElement(PopUp["default"], { className: cx("".concat(ns, "DatePicker-popup DatePicker-mobile")), container: popOverContainer, isShow: isOpened, showClose: false, onHide: this.handleClick },
