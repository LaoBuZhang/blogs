...
var ContextMenu = /** @class */ (function (_super) {
    ...
    ContextMenu.getInstance = function () {
        if (!ContextMenu.instance) {
            ...
            ReactDOM.render(React__default["default"].createElement(ThemedContextMenu, null), div);
        }
    ...
    ContextMenu.prototype.renderMenus = function (menus) {
        ...
        return menus.map(function (item, index) {
            if (item === '|') {
                return React__default["default"].createElement("li", { key: index, className: cx('ContextMenu-divider') });
            }
            var hasChildren = Array.isArray(item.children) && item.children.length;
            return (React__default["default"].createElement("li", { key: "".concat(item.label, "-").concat(index), className: cx('ContextMenu-item', item.className, {
                    'has-child': hasChildren,
                    'is-disabled': item.disabled
                }) },
                React__default["default"].createElement("a", { onClick: _this.handleClick.bind(_this, item), onMouseEnter: _this.handleMouseEnter.bind(_this, item), onMouseLeave: _this.handleMouseLeave.bind(_this, item) },
                    item.icon ? (React__default["default"].createElement("span", { className: cx('ContextMenu-itemIcon', item.icon) })) : null,
                    item.label),
                hasChildren ? (React__default["default"].createElement("ul", { className: cx('ContextMenu-subList') }, _this.renderMenus(item.children))) : null));
        });
    };
    ContextMenu.prototype.render = function () {
        var _this = this;
        var _a = this.props, className = _a.className; _a.container; var cx = _a.classnames;
        return (React__default["default"].createElement(Transition__default["default"], { mountOnEnter: true, unmountOnExit: true, onEnter: this.handleEnter, in: this.state.isOpened, timeout: 500 }, function (status) { return (React__default["default"].createElement("div", { ref: _this.menuRef, role: "contextmenu", className: cx('ContextMenu', {
                'ContextMenu--left': _this.state.align === 'left'
            }, className), onContextMenu: _this.handleSelfContextMenu },
            React__default["default"].createElement("div", { className: cx("ContextMenu-overlay", fadeStyles[status]) }),
            React__default["default"].createElement("div", { className: cx("ContextMenu-cursor"), style: { left: "".concat(_this.state.x, "px"), top: "".concat(_this.state.y, "px") } }),
            React__default["default"].createElement("div", { style: { left: "".concat(_this.state.x, "px"), top: "".concat(_this.state.y, "px") }, className: cx("ContextMenu-menu", fadeStyles[status]) },
                React__default["default"].createElement("ul", { className: cx('ContextMenu-list') }, _this.renderMenus(_this.state.menus))))); }));
    };
