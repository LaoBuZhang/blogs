...
var ToastComponent = /** @class */ (function (_super) {
    ...
    ToastComponent.prototype.notifiy = function (level, content, config) {
        var useMobileUI = (config.useMobileUI || this.props.useMobileUI) && amisCore.isMobile();
        this.setState(function (state) {
            ...
            items.push(tslib.__assign(tslib.__assign({ body: content, level: level }, config), { id: amisCore.guid(), position: config.position || (useMobileUI ? 'center' : config.position), timeout: config.timeout || (useMobileUI ? 3000 : undefined) }));
            return {
                items: items,
    ...
    ToastComponent.prototype.render = function () {
        ...
        var mobileUI = (useMobileUI || this.state.useMobileUI) && amisCore.isMobile();
        ...
        return Object.keys(groupedItems).map(function (position) {
            ...
            return (React__default["default"].createElement("div", { key: position, className: cx("Toast-wrap Toast-wrap--".concat(position.replace(/\-(\w)/g, function (_, l) {
...
var ToastMessage = /** @class */ (function (_super) {
    ...
    ToastMessage.prototype.render = function () {
        var _this = this;
        var _a = this.props, onDismiss = _a.onDismiss, cx = _a.classnames, closeButton = _a.closeButton, title = _a.title, body = _a.body, allowHtml = _a.allowHtml, level = _a.level, showIcon = _a.showIcon, useMobileUI = _a.useMobileUI; _a.translate;
        var iconName = useMobileUI ? '' : 'alert-';
        return (React__default["default"].createElement(Transition__default["default"], { mountOnEnter: true, unmountOnExit: true, in: this.state.visible, timeout: 750, onEntered: this.handleEntered, onExited: onDismiss }, function (status) {
            var _a, _b, _c, _d;
            return (React__default["default"].createElement("div", { className: cx("Toast Toast--".concat(level), fadeStyles[status], {
                ...
                showIcon === false ? null : (React__default["default"].createElement("div", { className: cx('Toast-icon') }, level === 'success' ? (React__default["default"].createElement(icons.Icon, { icon: iconName + 'success', className: "icon" })) : level == 'error' ? (React__default["default"].createElement(icons.Icon, { icon: iconName + 'fail', className: "icon" })) : level == 'info' ? (React__default["default"].createElement(icons.Icon, { icon: iconName + 'info', className: "icon" })) : level == 'warning' ? (React__default["default"].createElement(icons.Icon, { icon: iconName + 'warning', className: "icon" })) : null)),
                React__default["default"].createElement("div", { className: cx('Toast-content') },
                    typeof title === 'string' ? (React__default["default"].createElement("span", { className: cx("Toast-title") }, title)) : React__default["default"].isValidElement(title) ? (React__default["default"].cloneElement(title, {
                    ...
                    })) : typeof body === 'string' || typeof body === 'object' ? (React__default["default"].createElement("div", { className: cx('Toast-body') }, allowHtml ? (React__default["default"].createElement(Html["default"], { html: body === null || body === void 0 ? void 0 : body.toString() })) : (body === null || body === void 0 ? void 0 : body.toString()))) : null),
                closeButton ? (React__default["default"].createElement("a", { onClick: _this.close, className: cx("Toast-close") },
                    React__default["default"].createElement(icons.Icon, { icon: "status-close", className: "icon" }))) : null));
        }));
    };
