...
var Collapse = /** @class */ (function (_super) {
    ...
    Collapse.prototype.render = function () {
        ...
        var dom = [
            finalHeader ? (React__default["default"].createElement(HeadingComponent, { key: "header", onClick: this.toggleCollapsed, className: cx("Collapse-header", headingClassName) },
                showArrow && collapsable ? (expandIcon ? (React__default["default"].cloneElement(expandIcon, tslib.__assign(tslib.__assign({}, expandIcon.props), { className: cx('Collapse-icon-tranform', (_b = expandIcon.props) === null || _b === void 0 ? void 0 : _b.className) }))) : (React__default["default"].createElement("span", { className: cx('Collapse-arrow-wrap') },
                    React__default["default"].createElement(icons.Icon, { icon: "right-arrow-bold", className: cx('Collapse-arrow', 'icon'), wrapClassName: cx('Collapse-arrow'), iconContent: "Collapse-arrow" })))) : (''),
                finalHeader)) : null,
            React__default["default"].createElement(Transition__default["default"], { key: "body", mountOnEnter: mountOnEnter, unmountOnExit: unmountOnExit, in: !this.state.collapsed, timeout: 300, onEnter: this.handleEnter, onEntering: this.handleEntering, onEntered: this.handleEntered, onExit: this.handleExit, onExiting: this.handleExiting }, function (status) {
                if (status === Transition.ENTERING) {
                    _this.contentDom.offsetWidth;
                }
                return (React__default["default"].createElement("div", { className: cx('Collapse-contentWrapper', collapseStyles[status]), ref: _this.contentRef },
                    React__default["default"].createElement("div", { className: cx('Collapse-body', bodyClassName) },
                        React__default["default"].createElement("div", { className: cx('Collapse-content') }, body || children))));
            })
        ];
        if (headerPosition === 'bottom') {
            dom.reverse();
        }
        return (React__default["default"].createElement(WrapperComponent, { className: cx("Collapse", (_a = {
