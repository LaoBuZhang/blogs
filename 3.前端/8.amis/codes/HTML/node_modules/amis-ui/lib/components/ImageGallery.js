/**
 * amis-ui v2.7.2
 * Copyright 2018-2023 fex
 */

'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var tslib = require('tslib');
var React = require('react');
var debounce = require('lodash/debounce');
var amisCore = require('amis-core');
var Modal = require('./Modal.js');
var icons = require('./icons.js');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var React__default = /*#__PURE__*/_interopDefaultLegacy(React);
var debounce__default = /*#__PURE__*/_interopDefaultLegacy(debounce);

exports.ImageActionKey = void 0;
(function (ImageActionKey) {
    /** 右旋转 */
    ImageActionKey["ROTATE_RIGHT"] = "rotateRight";
    /** 左旋转 */
    ImageActionKey["ROTATE_LEFT"] = "rotateLeft";
    /** 等比例放大 */
    ImageActionKey["ZOOM_IN"] = "zoomIn";
    /** 等比例缩小 */
    ImageActionKey["ZOOM_OUT"] = "zoomOut";
    /** 恢复原图缩放比例尺 */
    ImageActionKey["SCALE_ORIGIN"] = "scaleOrigin";
})(exports.ImageActionKey || (exports.ImageActionKey = {}));
var ImageGallery = /** @class */ (function (_super) {
    tslib.__extends(ImageGallery, _super);
    function ImageGallery() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.state = {
            isOpened: false,
            index: -1,
            items: [],
            scale: 1,
            rotate: 0,
            showToolbar: false,
            actions: ImageGallery.defaultProps.actions
        };
        _this.handleToolbarAction = debounce__default["default"](function (action) {
            if (action.disabled) {
                return;
            }
            switch (action.key) {
                case exports.ImageActionKey.ROTATE_LEFT:
                    _this.setState(function (prevState) { return ({ rotate: prevState.rotate - 90 }); });
                    break;
                case exports.ImageActionKey.ROTATE_RIGHT:
                    _this.setState(function (prevState) { return ({ rotate: prevState.rotate + 90 }); });
                    break;
                case exports.ImageActionKey.ZOOM_IN:
                    _this.setState(function (prevState) { return ({ scale: prevState.scale + 0.5 }); });
                    break;
                case exports.ImageActionKey.ZOOM_OUT:
                    _this.setState(function (prevState) {
                        return prevState.scale - 0.5 > 0
                            ? { scale: prevState.scale - 0.5 }
                            : null;
                    });
                    break;
                case exports.ImageActionKey.SCALE_ORIGIN:
                    _this.setState(function () { return ({ scale: 1 }); });
                    break;
            }
            if (action.onClick && typeof action.onClick === 'function') {
                action.onClick(_this);
            }
        }, 250, { leading: true, trailing: false });
        return _this;
    }
    ImageGallery.prototype.galleryMainRef = function (ref) {
        var _a;
        if (ref) {
            ref.addEventListener('wheel', this.onWheelScroll, {
                passive: false
            });
        }
        else {
            (_a = this.galleryMain) === null || _a === void 0 ? void 0 : _a.removeEventListener('wheel', this.onWheelScroll);
        }
        this.galleryMain = ref;
    };
    ImageGallery.prototype.onWheelScroll = function (event) {
        var _a;
        var showToolbar = (_a = this.state) === null || _a === void 0 ? void 0 : _a.showToolbar;
        if (!showToolbar) {
            return;
        }
        event.preventDefault();
        /** 向上滚动放大，向下滚动缩小 */
        if (event.deltaY > 0) {
            this.handleToolbarAction({ key: 'zoomOut' });
        }
        else if (event.deltaY < 0) {
            this.handleToolbarAction({ key: 'zoomIn' });
        }
    };
    ImageGallery.prototype.handleImageEnlarge = function (info) {
        var actions = this.props.actions;
        var validActionKeys = Object.values(exports.ImageActionKey);
        this.setState({
            isOpened: true,
            items: info.list ? info.list : [info],
            index: info.index || 0,
            /* children组件可以控制工具栏的展示 */
            showToolbar: !!info.showToolbar,
            /** 外部传入合法key值的actions才会生效 */
            actions: Array.isArray(info.toolbarActions)
                ? info.toolbarActions.filter(function (action) {
                    return validActionKeys.includes(action === null || action === void 0 ? void 0 : action.key);
                })
                : actions
        });
    };
    ImageGallery.prototype.resetImageAction = function () {
        this.setState({ scale: 1, rotate: 0 });
    };
    ImageGallery.prototype.close = function () {
        this.setState({
            isOpened: false
        });
        this.resetImageAction();
    };
    ImageGallery.prototype.prev = function () {
        var index = this.state.index;
        this.setState({ index: index - 1 });
        this.resetImageAction();
    };
    ImageGallery.prototype.next = function () {
        var index = this.state.index;
        this.setState({ index: index + 1 });
        this.resetImageAction();
    };
    ImageGallery.prototype.handleItemClick = function (e) {
        var index = parseInt(e.currentTarget.getAttribute('data-index'), 10);
        this.setState({ index: index });
        this.resetImageAction();
    };
    ImageGallery.prototype.renderToolbar = function (actions) {
        var _this = this;
        var _a = this.props, cx = _a.classnames, __ = _a.translate, className = _a.className;
        var scale = this.state.scale;
        return (React__default["default"].createElement("div", { className: cx('ImageGallery-toolbar', className) }, actions.map(function (action) { return (React__default["default"].createElement("div", { className: cx('ImageGallery-toolbar-action', {
                'is-disabled': action.disabled ||
                    (action.key === exports.ImageActionKey.ZOOM_OUT && scale - 0.5 <= 0)
            }), key: action.key, onClick: function () { return _this.handleToolbarAction(action); } },
            React__default["default"].createElement("a", { className: cx('ImageGallery-toolbar-action-icon'), "data-tooltip": __(action.label), "data-position": "top" }, React__default["default"].isValidElement(action.icon) ? (React__default["default"].cloneElement(action.icon, {
                className: cx('icon', action.iconClassName)
            })) : (React__default["default"].createElement(icons.Icon, { icon: action.icon, className: cx('icon', action.iconClassName) }))))); })));
    };
    ImageGallery.prototype.render = function () {
        var _this = this;
        var _a = this.props, children = _a.children, cx = _a.classnames, modalContainer = _a.modalContainer;
        var _b = this.state, index = _b.index, items = _b.items, rotate = _b.rotate, scale = _b.scale, showToolbar = _b.showToolbar, actions = _b.actions;
        var __ = this.props.translate;
        return (React__default["default"].createElement(React__default["default"].Fragment, null,
            React__default["default"].cloneElement(children, {
                onImageEnlarge: this.handleImageEnlarge
            }),
            React__default["default"].createElement(Modal["default"], { closeOnEsc: true, size: "full", onHide: this.close, show: this.state.isOpened, contentClassName: cx('ImageGallery'), container: modalContainer },
                React__default["default"].createElement("a", { "data-tooltip": __('Dialog.close'), "data-position": "left", className: cx('ImageGallery-close'), onClick: this.close },
                    React__default["default"].createElement(icons.Icon, { icon: "close", className: "icon" })),
                ~index && items[index] ? (React__default["default"].createElement(React__default["default"].Fragment, null,
                    React__default["default"].createElement("div", { className: cx('ImageGallery-title') }, items[index].title),
                    React__default["default"].createElement("div", { className: cx('ImageGallery-main'), ref: this.galleryMainRef },
                        React__default["default"].createElement("img", { src: items[index].originalSrc, style: { transform: "scale(".concat(scale, ") rotate(").concat(rotate, "deg)") } }),
                        showToolbar && Array.isArray(actions) && actions.length > 0
                            ? this.renderToolbar(actions)
                            : null,
                        items.length > 1 ? (React__default["default"].createElement(React__default["default"].Fragment, null,
                            React__default["default"].createElement("a", { className: cx('ImageGallery-prevBtn', index <= 0 ? 'is-disabled' : ''), onClick: this.prev },
                                React__default["default"].createElement(icons.Icon, { icon: "prev", className: "icon" })),
                            React__default["default"].createElement("a", { className: cx('ImageGallery-nextBtn', index >= items.length - 1 ? 'is-disabled' : ''), onClick: this.next },
                                React__default["default"].createElement(icons.Icon, { icon: "next", className: "icon" })))) : null))) : null,
                items.length > 1 ? (React__default["default"].createElement("div", { className: cx('ImageGallery-footer') },
                    React__default["default"].createElement("a", { className: cx('ImageGallery-prevList is-disabled') },
                        React__default["default"].createElement(icons.Icon, { icon: "prev", className: "icon" })),
                    React__default["default"].createElement("div", { className: cx('ImageGallery-itemsWrap') },
                        React__default["default"].createElement("div", { className: cx('ImageGallery-items') }, items.map(function (item, i) { return (React__default["default"].createElement("div", { key: i, "data-index": i, onClick: _this.handleItemClick, className: cx('ImageGallery-item', i === index ? 'is-active' : '') },
                            React__default["default"].createElement("img", { src: item.src }))); }))),
                    React__default["default"].createElement("a", { className: cx('ImageGallery-nextList is-disabled') },
                        React__default["default"].createElement(icons.Icon, { icon: "next", className: "icon" })))) : null)));
    };
    ImageGallery.defaultProps = {
        actions: [
            {
                key: exports.ImageActionKey.ROTATE_LEFT,
                icon: 'rotate-left',
                label: 'rotate.left'
            },
            {
                key: exports.ImageActionKey.ROTATE_RIGHT,
                icon: 'rotate-right',
                label: 'rotate.right'
            },
            { key: exports.ImageActionKey.ZOOM_IN, icon: 'zoom-in', label: 'zoomIn' },
            { key: exports.ImageActionKey.ZOOM_OUT, icon: 'zoom-out', label: 'zoomOut' },
            {
                key: exports.ImageActionKey.SCALE_ORIGIN,
                icon: 'scale-origin',
                label: 'scale.origin'
            }
        ]
    };
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", [HTMLDivElement]),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "galleryMainRef", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", [WheelEvent]),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "onWheelScroll", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", [Object]),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "handleImageEnlarge", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", []),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "close", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", []),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "prev", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", []),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "next", null);
    tslib.__decorate([
        amisCore.autobind,
        tslib.__metadata("design:type", Function),
        tslib.__metadata("design:paramtypes", [Object]),
        tslib.__metadata("design:returntype", void 0)
    ], ImageGallery.prototype, "handleItemClick", null);
    return ImageGallery;
}(React__default["default"].Component));
var ImageGallery$1 = amisCore.themeable(amisCore.localeable(ImageGallery));

exports.ImageGallery = ImageGallery;
exports["default"] = ImageGallery$1;
