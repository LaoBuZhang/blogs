...
import { eachTree, getTreeAncestors, getTreeParent, findTreeIndex, isVisible, highlight, hasAbility, autobind, themeable, localeable } from 'amis-core';
...
var TreeSelector = /** @class */ (function (_super) {
    ...
    TreeSelector.prototype.handleAdd = function (parent) {
        ...
        if (!bultinCUD) {
            ...
            this.setState({
                ...
                var result = [];
                for (var _i = 0, _a = _this.state.flattenedOptions; _i < _a.length; _i++) {
                    var option = _a[_i];
                    result.push(option);
                    if (option === parent) {
                        result.push(__assign(__assign({}, option), { isAdding: true }));
                    }
                }
    ...
    TreeSelector.prototype.flattenOptions = function (props, initial) {
        ...
        eachTree((props === null || props === void 0 ? void 0 : props.options) || this.props.options, function (item, key, level, paths) {
            ...
            if (paths.length === 1) {
                ...
                // 父节点是展开的状态
                item.level = level;
                item.key = "".concat(parent.key, "-").concat(key);
                flattenedOptions.push(item);
            }
        });
        if (initial) {
    ...
    TreeSelector.prototype.renderItem = function (_a) {
        ...
        var iconValue = item[iconField] || (item.children ? 'folder' : 'file');
        ...
        if (isEditing && editingItem === item) {
            ...
            body = (React__default.createElement("div", { className: cx('Tree-itemLabel', {
                ...
                item.loading ? (React__default.createElement(Spinner, { size: "sm", show: true, icon: "reload", spinnerClassName: cx('Tree-spinner'), loadingConfig: loadingConfig })) : !isLeaf || (item.defer && !item.loaded) ? (React__default.createElement("div", { onClick: function () { return _this.toggleUnfolded(item); }, className: cx('Tree-itemArrow', {
                ...
                React__default.createElement("div", { className: cx('Tree-itemLabel-item') },
                    showIcon ? (React__default.createElement("i", { className: cx("Tree-itemIcon ".concat(item.children ? 'Tree-folderIcon' : 'Tree-leafIcon')), onClick: function () {
                            return !disabled &&
                                (multiple
                                    ? _this.handleCheck(item, !checked)
                                    : _this.handleSelect(item));
                        } }, getIcon(iconValue) ? (React__default.createElement(Icon, { icon: iconValue, className: "icon" })) : React__default.isValidElement(iconValue) ? (iconValue) : (React__default.createElement("i", { className: iconValue })))) : null,
                    ...
                    !disabled &&
                        !isAdding &&
                        !isEditing &&
                        !(item.defer && !item.loaded) ? (React__default.createElement("div", { className: cx('Tree-item-icons') },
                        creatable && hasAbility(item, 'creatable') ? (React__default.createElement("a", { onClick: this.handleAdd.bind(this, item), "data-tooltip": __(createTip), "data-position": "left" },
                            React__default.createElement(Icon, { icon: "plus", className: "icon" }))) : null,
                        removable && hasAbility(item, 'removable') ? (React__default.createElement("a", { onClick: this.handleRemove.bind(this, item), "data-tooltip": __(removeTip), "data-position": "left" },
                            React__default.createElement(Icon, { icon: "minus", className: "icon" }))) : null,
                        editable && hasAbility(item, 'editable') ? (React__default.createElement("a", { onClick: this.handleEdit.bind(this, item), "data-tooltip": __(editTip), "data-position": "left" },
                            React__default.createElement(Icon, { icon: "new-edit", className: "icon" }))) : null)) : null)));
        }
        return (React__default.createElement("li", { key: item.key, className: cx("Tree-item ".concat(itemClassName || ''), {
                'Tree-item--isLeaf': isLeaf,
                'is-child': this.relations.get(item)
            }), style: __assign(__assign({}, style), { left: "calc(".concat(level, " * var(--Tree-indent))"), width: "calc(100% - ".concat(level, " * var(--Tree-indent))") }) }, body));
    };
    TreeSelector.prototype.renderList = function (list, value) {
        ...
        if (virtualThreshold && list.length > virtualThreshold) {
            return (React__default.createElement(VirtualList, { height: list.length > 8 ? 266 : list.length * itemHeight, itemCount: list.length, itemSize: itemHeight, 
                //! hack: 让 VirtualList 重新渲染
                renderItem: this.renderItem.bind(this) }));
        }
        return list.map(function (item, index) { return _this.renderItem({ index: index }); });
    };
