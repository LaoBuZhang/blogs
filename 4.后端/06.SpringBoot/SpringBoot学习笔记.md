...
运行 `SpringBoot` 工程不需要使用本地的 `Tomcat` 和 插件，只运行项目 `com.itheima` 包下的 `Application` 类，我们就可以在控制台看出如下信息，该类被称为**启动类**
...
`dependencyManagement` 标签是进行**依赖版本锁定**，但是并没有导入对应的依赖；如果我们工程需要那个依赖只需要引入依赖的 `groupid` 和 `artifactId` 不需要定义 `version`。
...
**数组数据**在数据书写位置的下方使用减号作为数据开始符号，每行书写一个数据，减号与数据间空格分隔，例如
...
**即在一个配置文件中添加多套配置，并且自动在不同的场景下使用不同的配置**
...
**application.properties是主配置文件，用来指定使用哪个子配置文件**
...
#### 2.5.5 Maven与SpringBoot关联操作

在pom.xml（代表maven的配置）和.yaml（代表SpringBoot的配置）文件中都可以指定端口号

此时会默认使用SpringBoot的配置
...
> ==说明：==级别越高优先级越高
>
> file表示target目录，可在compile后在此手动添加配置文件
...
**在SpringBoot中，main所在的类起到Spring配置文件的作用**
