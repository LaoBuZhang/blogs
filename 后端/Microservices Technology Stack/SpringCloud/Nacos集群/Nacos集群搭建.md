...
官方推荐的最佳实践是使用带有**主从的高可用数据库集群**。

这里我们以单点的数据库为例来讲解。

首先新建一个mysql数据库，命名为nacos，而后导入下面的SQL：
...
修改conf/nginx.conf文件，配置如下，粘贴在http内部即可：
...
使用start nginx.exe启动nginx，注意两点，一是80端口是否被占用，二是路径是否有中文，否则都会启动失败
